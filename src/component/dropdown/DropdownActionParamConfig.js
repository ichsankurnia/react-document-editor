...
const DropdownActionParamConfig = ({onEdit, onDelete, titleActive, onActive}) => {
    ...
    return (
        <div className="relative inline-flex justify-center item-center flex-row-reverse">
            ...
            {dropdownOpen &&
            <div className={`mr-1 md:m-0 md:absolute right-7 -bottom-10 bg-white rounded-md shadow-lg overflow-hidden p-1 border-1 border-gray-200`}>
                <div ref={dropdown} onFocus={() => setDropdownOpen(true)} onBlur={() => setDropdownOpen(false)} >
                    <div className="flex flex-col w-40 min-w-max text-left">
                        <button onClick={()=>showConfirmActive(true)} className="px-2 py-2 text-sm text-gray-700 hover:bg-red-800 hover:text-white inline-flex rounded-md items-center">
                            <i className={`${titleActive==='Activate'? 'ri-shield-check-fill': 'ri-close-circle-fill'} mr-3 mt-0.5`} />
                            <p>{titleActive}</p>
                        </button>
            ...
            {confirmDelete && <ModalConfirm message='Delete this config?' onOK={handleDelete} onCancel={() => showConfirmDelete(false)} />}
            {confirmActive && <ModalConfirm message={titleActive==='Activate'?'Activate this config?':'Deactivate this config?'} onOK={handleActivateUser} onCancel={() => showConfirmActive(false)} />}
        </div>
    )
}

export default DropdownActionParamConfig